...
class StoreAllocationACO:
    ...
    def _init_numpy_mappings(self):
        if StoreAllocationACO._cached_mappings is not None:
            (self.s2i, self.i2s, self.np_dist, self.np_time,
             ...
             self.np_latest, self.np_region, self.np_sched) = StoreAllocationACO._cached_mappings
            return
    ...
    def _generate_greedy_solution(self, return_routes=False):
        ...
        if return_routes:
            ...
            for i, target in enumerate(greedy_choices):
                if target != 'SUPPORT':
                    store = self.remaining_stores[i]
                    _, pos = self._get_store_insertion_cost_and_pos(rm.routes_info[target], store)
                    rm.insert_store(store, target, pos, fast_update=True)
            return greedy_choices, total_cost, rm.routes_info, support_pool, vn
